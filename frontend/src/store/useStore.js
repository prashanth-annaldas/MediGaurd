...
const useStore = create((set, get) => ({
    ...
    fetchPrediction: async (currentStats) => {
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/predict_shortage`, {
                method: 'POST',
    ...
    addAlert: async (alertData) => {
        ...
        // Persist to DB
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/alerts`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    deleteAlert: async (id) => {
        ...
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/alerts/${id}`, { method: 'DELETE' });
        } catch (e) {
    ...
    clearAllAlerts: async () => {
        ...
        try {
            ...
            const url = hospitalName ? `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/alerts?hospital_name=${encodeURIComponent(hospitalName)}` : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/alerts`;
